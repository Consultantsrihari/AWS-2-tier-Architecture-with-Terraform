...
resource "aws_internet_gateway" "SH_IGW" {
  vpc_id = aws_vpc.SH_VPC.id
}
...
resource "aws_eip" "NAT_eip" {
  domain     = "vpc"
  depends_on = [aws_internet_gateway.SH_IGW]
}

# Create NAT Gateway for Private Subnets
resource "aws_nat_gateway" "SH_NATGW" {
...
resource "aws_route_table" "SH_Public_Route" {
  vpc_id = aws_vpc.SH_VPC.id

  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.SH_IGW.id
  }
}

# Create Private Route Table
resource "aws_route_table" "SH_Private_Route" {
  vpc_id = aws_vpc.SH_VPC.id

  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.SH_NATGW.id
  }
}
...
resource "aws_route_table_association" "Public1" {
  subnet_id      = aws_subnet.Public1.id
  route_table_id = aws_route_table.SH_Public_Route.id
}
...
resource "aws_route_table_association" "Private1" {
  ...
  route_table_id = aws_route_table.SH_Private_Route.id
}
